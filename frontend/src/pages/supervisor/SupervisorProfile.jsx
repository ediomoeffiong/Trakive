/**
 * @file SupervisorProfile.jsx
 * @description Supervisor Profile & Account Management page.
...
const SupervisorOverviewTab = ({ setActiveTab }) => {
      ...
      >
        ...
        <div className="card p-6 profile-about-card" tabIndex={0}>
          ...
          <div>
            ...
            <InfoRow
              label="Role"
              value={`${profile?.jobTitle || profile?.role || 'Supervisor'}${profile?.department ? ` (${profile.department})` : ''}`}
            />
            <InfoRow label="Location" value={locationStr || null} placeholder="No location added" />
            <InfoRow label="Date Joined" value={profile?.dateJoined ? formatDate(profile.dateJoined) : null} />
          </div>
        </div>

        {/* Supervision Details card */}
        <div className="card p-6">
          <h3 style={{ fontSize: '0.875rem', fontWeight: 700, color: 'var(--color-neutral-700)', marginBottom: '0.875rem' }}>
            Supervision Details
          </h3>
          <div>
            <InfoRow label="Supervisor ID" value={profile?.employeeId} />
            <InfoRow label="Role & Tier" value={profile?.role} />
...
const SupervisorProfilePage = () => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'overview':
        return <SupervisorOverviewTab setActiveTab={setActiveTab} />;
      case 'personal':
  ...
  return (
    <motion.div className="profile-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.35 }}>
      <h1 className="sr-only">Supervisor Profile - Trakive</h1>

      {loadingProfile ? <ProfileHeaderSkeleton /> : <ProfileHeader completion={completion} isSupervisorOverride />}
